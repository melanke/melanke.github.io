...
`content/timeline-items.ts` (per-item `role`, `description`, `technologies`,
`priority` per version, and `printIn` — the list of versions whose PDF the item
reaches). Adding a version means touching those plus `app/<route>/page.tsx`,
`app/sitemap.ts` and the `VERSIONS` list in `scripts/print-cv.mjs`.

What the CV must **not** claim lives in `content/career-gaps.md` — the negative
record of what Gil never used, used long ago, or has but hasn't registered yet.
Read it before adding any skill the timeline does not already support.
